  ...
  export default {
    ...
    data () {
      return {
        ...
        lockX:true,
        lockY:false,
        height: common.getDeviceHeight(1.2), // -60
        pagination: {
          pageNo: 0,
          pageSize: 10
        },
    ...
    methods: {
      ...
      loadData(){
        ...
        let params = {
          interfaceId: common.interfaceIds.getMypartInActs,
      ...
      setData(data){
        ...
        if(_self.pagination.pageNo == 0) {
          _self.activitys = activitys;
        } else {
          _self.activitys.push(...data.activitys);
        }
        _self.loadMoreStatus.show=false;
        ...
        _self.$refs.scroller.donePullup();
        //判断数据是否有一页
        if(activitys.length < _self.pagination.pageSize){
          _self.hasMore = false;
