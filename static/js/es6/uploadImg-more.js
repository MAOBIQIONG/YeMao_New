...
const uploadImg2 ={
          ...
          for (var i=0;i<len;i++) {
            (function () {
              ...
              plus.io.resolveLocalFileSystemURL(files[i],function(entry){
                //图片：entry.toLocalURL()
                // if( uploadImg2.imgArr.indexOf(entry.toLocalURL()) < 0 ){
  ...
  getOrientation:function (imgPath,index){
    console.log("getOrientation index:"+index)
  ...
  clearImgArr:function(flag,path) {
    if( flag == true ){//清空所有图片及缓存
      ...
      var index = uploadImg2.imgArr.indexOf(path);
      console.log("clearImgArr:"+index)
      if( index >= 0 ){
        uploadImg2.imgArr.splice(index,1);//清除图片路径
        uploadImg2.imgBase64.splice(index,1);//清除图片路径
      }
